/**
 * Smart Pen ESP32 - WiFi Mode Only (Updated for Dynamic IP)
...
// =============================================
// These will be configured via AP setup mode or EEPROM
String LAPTOP_IP = "192.168.1.100";          // Default - will be updated
const int LAPTOP_PORT = 3005;                // Backend server port
const char* WIFI_ENDPOINT = "/api/mpu6050/readings";

// WiFi credentials - Will be set via AP setup mode
String WIFI_SSID = "YourWiFiNetwork";        // Will be updated via setup
String WIFI_PASSWORD = "";                   // Will be updated via setup
...
String commonIPs[] = {
  ...
  "192.168.2.13", "192.168.2.100", "192.168.2.101",
...
void setup() {
  ...
  Serial.println("🦡 Smart Pen ESP32 - WiFi Mode v2.0");
  Serial.println("📶 Dynamic IP discovery enabled");
...
void discoverServerIP() {
  Serial.println("🔍 Starting IP discovery process...");
...
void setupConfigServer() {
  server.on("/", HTTP_GET, []() {
    ...
    html += "<label>Laptop IP Address:</label><input type='text' name='laptop_ip' value='" + LAPTOP_IP + "' placeholder='192.168.1.100' required><br>";
    ...
    html += "<ol><li>Find your computer's IP address (ipconfig/ifconfig)</li>";
...
void handleButton() {
  ...
  if (currentButtonState && !buttonPressed && (millis() - lastButtonPress > BUTTON_DEBOUNCE)) {
    ...
    if (configurationMode) {
      Serial.println("🔄 Button pressed - restarting to retry WiFi connection");
      ESP.restart();
    } else if (wifiConnected) {
